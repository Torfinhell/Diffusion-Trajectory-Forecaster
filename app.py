...
from src.utils.training import _configure_model_shapes, _resolve_model_dims
...
def load_scenarios(cfg, split: str, n: int) -> list[dict]:
    ...
    scenarios = []
    for sample in WebDatasetGrainSource(ds):
        if len(scenarios) >= n:
            break
...
def load_model(cfg, checkpoint_path: str):
    # Patch runtime-resolved input/output dims into the model config exactly as
    # training does, so the architecture matches the saved checkpoint. Without
    # this, model.denoise_shape / se_args.time_len are missing and instantiate
    # fails (app.yaml carries no trainer-derived shapes on its own).
    #
    # A model config may pin these shapes explicitly (e.g. a checkpoint trained
    # with a non-default current_index/action_len). When denoise_shape is already
    # set we trust the config verbatim and skip auto-resolution, so the
    # instantiated architecture matches exactly what was saved.
    if cfg.model.get("denoise_shape") is None:
        _configure_model_shapes(cfg.model, _resolve_model_dims(cfg))
...
def run_inference_batch(
    ...
    def infer_one(sample_key, single_batch):
        ref = BaseTrainer._past_ref_coords(
            single_batch["agent_past"], single_batch["agent_past_valid"]
        )
        past_path = AgentPath(single_batch["agent_past"], action_len, ref_coords=ref)
        future_path = AgentPath(single_batch["agent_future"], action_len, ref_coords=ref)
        model_batch = {k: v for k, v in single_batch.items() if k != "agent_future"}
        sampled = BaseTrainer.sample_one_sol(
            model, diffusion_sampler, data_shape, model_batch, sample_key, action_len
        )
        if extract_actions:
            local_xy = past_path.decode_action_sample(
                sampled,
                accel_scale=float(app_cfg.accel_scale),
                yaw_rate_scale=float(app_cfg.yaw_rate_scale),
            )
        else:
            local_xy = future_path.decode_xy_sample(
                sampled,
                coord_scale=float(app_cfg.coord_scale),
                past_path=past_path,
            )
        # local_xy: (A, T, 2) in agent-local frame — rotate back to world frame
        x0 = ref[..., 0, None]   # (A, 1)
        y0 = ref[..., 1, None]
        cos_t = jnp.cos(ref[..., 2, None])
        sin_t = jnp.sin(ref[..., 2, None])
        world_x = local_xy[..., 0] * cos_t - local_xy[..., 1] * sin_t + x0
        world_y = local_xy[..., 0] * sin_t + local_xy[..., 1] * cos_t + y0
        return jnp.stack([world_x, world_y], axis=-1)
        ...
        while True:
            ...
            if all_indices:
                try:
                    ...
                    for i, pred_xy_world in zip(all_indices, results):
                        s = self.scenarios[i]
                        pred_xy_plot = np.asarray(pred_xy_world)
