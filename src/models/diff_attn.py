...
class AttentionMLP(eqx.Module):
    ...
    def __call__(self, x, kv_cond=None, attn_mask=None):
        if self.pre_norm:
            ...
            if self.type_attn == "self":
                x = x + self.attn(
                    normed, normed, normed, mask=attn_mask, key=self.dropout_key
                )
            else:
                x = x + self.attn(
                    normed, kv_cond, kv_cond, mask=attn_mask, key=self.dropout_key
                )
    ...
    def __init__(
        ...
        if layers == 0:
            ...
            for layer_key in layer_keys:
                ...
                layer = eqx.tree_at(
                    lambda l: l.attn.output_proj.weight,
                    layer,
                    jnp.zeros_like(layer.attn.output_proj.weight),
                )
                layer = eqx.tree_at(
                    lambda l: l.mlp.layers[-1].weight,
                    layer,
                    jnp.zeros_like(layer.mlp.layers[-1].weight),
                )
                layer = eqx.tree_at(
                    lambda l: l.mlp.layers[-1].bias,
                    layer,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if relations is not None:
            ...
            scene_rel = self.relation_encoder(
                agent_relations, pair_mask
            )  # (a, rel_dim)
