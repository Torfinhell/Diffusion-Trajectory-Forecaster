...
class DebuggableBaseDiffusionModel(BaseDiffusionModel):
    ...
    def _update_metrics_for_batch(self, metrics, batch, return_first_prediction=False):
        ...
        for sample_idx in range(batch["agent_future"].shape[0]):
            ...
            pred_xy = self.sample_multiple_sol(
                ...
                oracle_gt_xy=(
                    gt_xy if self._oracle_enabled("use_for_sampling") else None
                ),
            )
            agents_coeffs = batch["agents_coeffs"][sample_idx]

            if debug_metrics:
                debug_metric_sample(sample_idx, pred_xy, gt_xy, agents_coeffs)
    ...
    def on_train_epoch_end(self) -> None:
        ...
        debug_denoiser_scale_enabled = bool(self.vis.get("debug_denoiser_scale", False))
