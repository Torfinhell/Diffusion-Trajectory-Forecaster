...
def plot_simulator_state(
    ...
    *,
    plot_all_trajectories: bool = True,
    pred_xy: Optional[np.ndarray] = None,  # any shape ending with (H, 2)
    pred_score: Optional[np.ndarray] = None,
    pred_alpha: float = 0.8,
    ...
    # fix colors for each agent
    A = traj.num_objects
    colors = plt.cm.tab20(np.linspace(0, 1, A))

    # ---------- draw background first (so GT is not covered) ----------
    ...
    # ---------- choose center and set axis early (for clipping preds) ----------
    current_xy = traj.xy[:, state.timestep, :]  # [A,2]
    if viz_config.center_agent_idx == -1:
        sdc_mask = np.asarray(state.object_metadata.is_sdc, dtype=bool)
        xy = current_xy[sdc_mask]
        if xy.shape[0] == 0:
            # fallback to any valid agent at this timestep
            vmask = np.asarray(traj.valid[:, state.timestep], dtype=bool)
            xy = current_xy[vmask]
            if xy.shape[0] == 0:
                xy = current_xy[:1]
    else:
        xy = current_xy[viz_config.center_agent_idx : viz_config.center_agent_idx + 1]

    origin_x, origin_y = float(xy[0, 0]), float(xy[0, 1])
    x_min = origin_x - viz_config.back_x
    x_max = origin_x + viz_config.front_x
    y_min = origin_y - viz_config.back_y
    y_max = origin_y + viz_config.front_y
    ax.axis((x_min, x_max, y_min, y_max))

    # ---------- GT: plot via Waymax (boxes etc.) ----------
    if plot_all_trajectories:
        try:
            is_controlled = np.asarray(traj.valid[:, state.timestep], dtype=bool)
        except Exception:
            is_controlled = np.asarray(state.object_metadata.is_valid, dtype=bool)
    else:
        is_controlled = datatypes.get_control_mask(state.object_metadata, highlight_obj)

    plot_trajectory(ax, traj, is_controlled, time_idx=state.timestep, indices=indices)

    # ---------- GT: re-plot polylines on top (ensures visibility) ----------
    # (This avoids “GT missing” due to zorder/style inside plot_trajectory.)
    gt_x = np.asarray(traj.x)  # (A,T)
    gt_y = np.asarray(traj.y)
    gt_v = np.asarray(traj.valid, dtype=bool)
    A, T = gt_x.shape
    for a in range(A):
        v = gt_v[a]
        if not np.any(v):
            continue
        ax.plot(
            np.where(v, gt_x[a], np.nan),
            np.where(v, gt_y[a], np.nan),
    ...
    if pred_xy is not None:
        ...
        if pred_xy.shape[-1] != 2:
            raise ValueError(f"pred_xy must end with (..., H, 2); got {pred_xy.shape}")

        H = pred_xy.shape[-2]
        pred_flat = pred_xy.reshape((-1, H, 2))  # (N, H, 2)

        # alpha modulation by score (optional)
        if pred_score is not None:
            s = np.asarray(pred_score).reshape((-1,))
            if s.size != pred_flat.shape[0] and pred_xy.ndim >= 4:
                K = pred_xy.shape[0]
                if s.size == K and (pred_flat.shape[0] % K == 0):
                    s = np.repeat(s, pred_flat.shape[0] // K)
            if s.size == pred_flat.shape[0]:
                s_norm = (s - s.min()) / (s.max() - s.min() + 1e-8)
                alphas = 0.1 + (pred_alpha - 0.1) * s_norm
            else:
                alphas = np.full((pred_flat.shape[0],), pred_alpha, dtype=float)
        else:
            alphas = np.full((pred_flat.shape[0],), pred_alpha, dtype=float)

        # break lines at bad points (NaN/Inf) and optionally outside viewport
        for k in range(pred_flat.shape[0]):
            ...
            ax.plot(
                ...
                linestyle=pred_linestyle,  # dashed
